...
export default function Sculpture({material,paused,reset}:{material:number;paused:boolean;reset:number}) {
 ...
 useEffect(()=>{
  ...
  Promise.all([import('three'),import('three/examples/jsm/environments/RoomEnvironment.js')]).then(([T,{RoomEnvironment}])=>{
   ...
   let dragging=false,lastX=0,lastY=0,targetX=.3,targetY=-.45,pointerX=0,pointerY=0,visible=true;
   ...
   const keydown=(e:KeyboardEvent)=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();targetY+=e.key==='ArrowRight'?.2:e.key==='ArrowLeft'?-.2:0;targetX+=e.key==='ArrowDown'?.2:e.key==='ArrowUp'?-.2:0;}};
   container.addEventListener('pointerdown',down);container.addEventListener('pointermove',move);container.addEventListener('pointerup',up);container.addEventListener('pointercancel',up);container.addEventListener('pointerleave',leave);container.addEventListener('keydown',keydown);
   ...
   cleanup=()=>{cancelAnimationFrame(frame);observer.disconnect();visibility.disconnect();container.removeEventListener('pointerdown',down);container.removeEventListener('pointermove',move);container.removeEventListener('pointerup',up);container.removeEventListener('pointercancel',up);container.removeEventListener('pointerleave',leave);container.removeEventListener('keydown',keydown);renderer!.domElement.removeEventListener('webglcontextlost',contextLost);geometry.dispose();surface.dispose();orbitGeometry.dispose();orbitMaterial.dispose();satelliteGeometry.dispose();satelliteMaterial.dispose();dustGeometry.dispose();dustMaterial.dispose();environment.dispose();renderer!.dispose();renderer!.domElement.remove();};
  }).catch(()=>{renderer?.dispose();if(!stopped)setFailed(true);});
  return()=>{stopped=true;cleanup?.();};
 },[]);
 return <div ref={host} className={`sculpture ${ready?'is-ready':''} ${failed?'is-failed':''}`} role="img" tabIndex={0} aria-label="Interaktiv 3D haykal. Sudrab yoki klaviatura yo‘nalish tugmalari bilan aylantiring.">{failed&&<img className="sculpture-fallback" src="/orbit.webp" alt=""/>}{(!ready||failed)&&<div className="sculpture-status">{failed?'Bu qurilmada statik ko‘rinish.':'3D olam uyg‘onmoqda…'}</div>}</div>;
}
